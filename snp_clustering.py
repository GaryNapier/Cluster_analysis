...
def main_stats(args):
    ...
    print("Mean clusters size: %s" % (sum([len(x) for x in graph.clusters])/len(graph.clusters)))
...
parser_sub = subparsers.add_parser('vcf2clusters', help='Calculate stats', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
parser_sub = subparsers.add_parser('stats', help='Calculate stats', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser_sub.add_argument('graph')
parser_sub.set_defaults(func=main_stats)


parser_sub = subparsers.add_parser('annotate', help='Output program version and exit', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser_sub.add_argument('graph')
parser_sub.add_argument('csv')
parser_sub.add_argument('out')
parser_sub.set_defaults(func=main_add_meta)

parser_sub = subparsers.add_parser('view', help='Calculate stats', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser_sub.add_argument('graph')
parser_sub.set_defaults(func=main_view)

parser_sub = subparsers.add_parser('downsample', help='Calculate stats', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser_sub.add_argument('graph')
parser_sub.set_defaults(func=main_downsample)

parser_sub = subparsers.add_parser('largest_cluster', help='Calculate stats', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser_sub.add_argument('graph')
