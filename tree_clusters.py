...
def run(args):
    ...
    output_stats_file = args.output_stats_file
    ...
    ts.title.add_face(TextFace("Lineage %s" % lineage, fsize=len(t)/4),
                      column=0)  # Add title to tree
    ts.layout_fn = layout
    ...
    for node in t.traverse("preorder"):
        ...
        # Actually get lower triangle. Pathetic.
        subdist_within_lt = subdist_within_lt[np.tril_indices(len(subdist_within_lt), k=off_diag)]
        ...
        subdist_btwn = subdist_btwn.reshape(
            subdist_btwn.shape[0]*subdist_btwn.shape[1], )  # Turn to vector
        ...
        if do_t_test == 1:
            ...
            if get_outlers:
                ...
                # Unlist and get unique
                outliers_within = list(set(list(chain.from_iterable(outliers_within))))
            ...
            # Store effect size in node as attribute (so can export as newick).
            node.add_features(effect_size=round(abs(effect_sz), 2))
    ...
    for i, sublin in enumerate(sublins):
        # Subset samples by sublineage
        samps_sublin = lineages_master.loc[lineages_master["sub_lineage"] == sublin, "sample"]
        nst = NodeStyle()
...
def main():
    ...
    parser.add_argument("-df", help="Distance matrix file",
                        dest="dist_file", type=str, required=True)
    parser.add_argument("-of", help="Output file name for table of stats",
                        dest="output_stats_file", type=str, default=None)
    parser.add_argument("-n", help="Which cluster numbers interested in? Comma-separated with no spaces e.g. 1,2,3",
                        dest="clust_nums", type=str, default=None)
    parser.add_argument("-s", help="Min number of samples in clusters",
                        dest="node_cutoff", type=int, default=20)
    parser.add_argument("-b", help="Min bootstrap for clusters",
                        dest="bootstrap_cutoff", type=int, default=51)
    parser.add_argument("-r", help="R value cutoff (-1<0<1) for anosim",
                        dest="test_stat", type=float, default=0.5)
    parser.add_argument("-t", help="0 = don't do t-test, 1 = do t-test, default = 0",
                        dest="do_t_test", type=int, default=0)
    parser.add_argument("-p", help="0 = don't show plots, 1 = show plots, default = 0",
                        dest="do_plots", type=int, default=0)
    parser.add_argument("-a", help="0 = don't do anosim analysis, 1 = do anosim analysis",
                        dest="do_anosim_analysis", type=int, default=0)
    parser.add_argument("-c", help="0 = don't do cluster analysis, 1 = do clustering analysis",
                        dest="do_clustering_analysis", type=int, default=0)
    parser.add_argument("-m", help="Metadata file for clustering analysis",
                        dest="metadata", type=str, default="")
    parser.add_argument("-o", help="0 = don't do outlier analyses, 1 = do outlier analyses",
                        dest="get_outlers", type=int, default=0)
