...
Fasta_files <- sprintf("lin_%s/fasta/lin_%s_results.filt.snps.fa", 
...
write.csv(clusters, file = "all_lins/itol/fastbap_clusts.csv", row.names = F, quote = F)
...
n <- length(unique(clusters$clusters))
write.csv(data.frame(Type = rep("clusters", n), 
                     Value = sort(unique(clusters$clusters)), 
                     Colour = rainbow(n) ), 
...
system("python python_scripts/csv2itol.py all_lins/itol/fastbap_clusts.csv all_lins/itol/fastbaps_key.csv all_lins/itol/")
...
Using clusGap() to predict optimal clusters
